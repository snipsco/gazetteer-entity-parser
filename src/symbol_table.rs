...
impl GazetteerParserSymbolTable {
    ...
    pub fn add_symbol(&mut self, symbol: &str, force_add: bool) -> GazetteerParserResult<i32> {
        if force_add || !self.string_to_indices.contains_key(symbol) {
            self.index_to_string.push(symbol.to_string());
            let symbol_idx = (self.index_to_string.len() - 1) as i32;
    ...
    pub fn find_symbol(&self, symbol: &str) -> GazetteerParserResult<Option<&Vec<i32>>> {
        Ok(self.string_to_indices.get(symbol))
    }

    /// Find the unique index of a symbol, and raise an error if it has more than one index
    pub fn find_single_symbol(&self, symbol: &str) -> GazetteerParserResult<Option<i32>> {
    ...
    pub fn find_index(&self, idx: i32) -> GazetteerParserResult<String> {
    ...
    pub fn from_path<P: AsRef<Path>>(filename: P) -> GazetteerParserResult<GazetteerParserSymbolTable> {
        ...
        let mut string_to_indices: HashMap<String, Vec<i32>> = HashMap::new();
